...
import org.kzac.common.UUIDHelper;
...
public class PersonServiceMapImpl implements PersonService {
    ...
    @Override
    public List<PersonInfo> searchForPersons(Criteria criteria, ContextInfo contextInfo)
            throws DoesNotExistException,
            InvalidParameterException,
            MissingParameterException,
            OperationFailedException,
	...
	for(PersonInfo person : personMap.values()){
            if(criteria.getWhere().resolve(person)){
        	persons.add(person);
            }
        }
	return persons;
